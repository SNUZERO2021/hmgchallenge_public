...
int main(int argc, char **argv){
    ...
    for(int i=0;i<global_map_.global_segment_map.size();i++){
        ...
        g %= 256;
        b %= 256;
        img.at<Vec3b>(maxy - 1 - miny - py,px)[0] = 255-r;
        img.at<Vec3b>(maxy - 1 - miny - py,px)[1] = 255-g;
        img.at<Vec3b>(maxy - 1 - miny - py,px)[2] = 255-b;
    }
    printf("complete to color pixels\n");
    for(int i=0;i<cnt;i++){
        ...
        for(int j=0;j<x.size();j++){
            ...
            p.second -= miny;
            // printf("%d %d\n", p.first, p.second);
            circle(img, cv::Point(p.first,maxy - 1 - miny - p.second), 3, Scalar(255,0,0),-1);
